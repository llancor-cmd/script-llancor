...
menu_nextcloud_occ_pro(){
  while true; do
    clear
echo -e "${BOLD}${CYAN}=== MENÚ NEXTCLOUD - Turnkey ${YELLOW}v24.2.0 ${CYAN}===${NC}"
echo -e " ${YELLOW}↑"
...
nc_crear_usuario(){
    ...
    echo -e "${CYAN}   Reutilizando carpeta existente en DATA         ${RESET}"
    echo -e "${YELLOW}==================================================${RESET}"
    ...
    if [ -d "$USER_DIR" ]; then
        ...
        if [[ "$REUSE_DATA" =~ ^[sS]$ ]]; then
            ...
            echo
            echo -e "${CYAN}Limpiando residuos conflictivos...${RESET}"
            ...
            echo -e "${GREEN}✔ Residuos eliminados${RESET}"
    ...
    # =====================================================
    # RESTAURAR DATA USUARIO
    # =====================================================

    if [ "$RESTORE_USER" = "1" ]; then

        echo
        echo -e "${CYAN}Restaurando datos usuario...${RESET}"

        # =================================================
        # ELIMINAR ESTRUCTURA NUEVA CREADA POR NEXTCLOUD
        # =================================================

        rm -rf "$USER_DIR"

        # =================================================
        # RESTAURAR CARPETA ORIGINAL COMPLETA
        # =================================================
        ...
        if [ $? -ne 0 ]; then
            ...
            err "Error restaurando carpeta usuario"
        ...
        echo -e "${GREEN}✔ Datos restaurados${RESET}"
    ...
    # =====================================================
    # LIMPIAR CACHE NEXTCLOUD
    # =====================================================

    echo
    echo -e "${CYAN}Limpiando cache Nextcloud...${RESET}"

    sudo -u "$NC_USER" php "$NC_PATH/occ" files:cleanup >/dev/null 2>&1

    echo -e "${GREEN}✔ Cache limpiada${RESET}"

# =====================================================
# ESCANEAR SOLO SI SE RESTAURO DATA
# =====================================================

if [ "$RESTORE_USER" = "1" ]; then

    echo
    echo -e "${CYAN}Limpiando cache archivos...${RESET}"
    ...
    echo
    echo -e "${YELLOW}Usuario nuevo sin DATA para escanear${RESET}"
...
nc_eliminar_usuario(){
    ...
    while true; do
        ...
        select user in $OPTIONS; do
            ...
            # =================================================
            # RESPALDAR DATA SI SE CONSERVA
            # =================================================

            TEMP_BACKUP="${DATA_DIR}/.${user}_delete_backup_$(date +%s)"

            if [[ ! "$DEL_DATA" =~ ^[sS]$ ]] && [ -d "$USER_DIR" ]; then

                echo
                echo -e "${CYAN}Respaldando DATA usuario...${RESET}"

                mv "$USER_DIR" "$TEMP_BACKUP"

                if [ $? -ne 0 ]; then

                    err "No se pudo respaldar DATA"
                ...
                echo -e "${GREEN}✔ DATA respaldada${RESET}"
            ...
            if [[ "$DEL_DATA" =~ ^[sS]$ ]]; then
                ...
                if [ -d "$TEMP_BACKUP" ]; then
                    ...
                    echo -e "${GREEN}✔ DATA restaurada${RESET}"

                fi

                echo
                echo -e "${GREEN}✔ DATA conservada:${RESET} $USER_DIR"
...
seleccionar_usuario_nc(){
    ...
    if [ -z "$USERS" ]; then
        ...
        err "No hay usuarios disponibles"
    ...
    for USER in $USERS; do
        ...
        IS_ADMIN=$(sudo -u "$NC_USER" php "$NC_PATH/occ" \
            group:list "$USER" 2>/dev/null | grep -w admin)
...
nc_resetear_password(){
    ...
    echo
    echo -e "${YELLOW}==================================================${RESET}"
    echo -e "${YELLOW}         RESET PASSWORD NEXTCLOUD                 ${RESET}"
    echo -e "${YELLOW}==================================================${RESET}"
    ...
    echo -e "${CYAN}Modo seguro:${RESET} solicitará contraseña 2 veces"
    echo
...
nc_cambiar_password(){
    ...
    echo
    echo -e "${YELLOW}==================================================${RESET}"
    echo -e "${YELLOW}      CAMBIAR PASSWORD AUTOMATICO                 ${RESET}"
    echo -e "${YELLOW}==================================================${RESET}"
    ...
    echo
    echo -e "${CYAN}Aplicando contraseña...${RESET}"

    if sudo -u "$NC_USER" OC_PASS="$pass" php "$NC_PATH/occ" \
        user:resetpassword --password-from-env "$SELECTED_USER"; then
...
nc_hacer_admin(){
    ...
    echo
    echo -e "${YELLOW}==================================================${RESET}"
    echo -e "${YELLOW}          CONVERTIR EN ADMIN                      ${RESET}"
    echo -e "${YELLOW}==================================================${RESET}"
    ...
    if sudo -u "$NC_USER" php "$NC_PATH/occ" \
        group:list "$SELECTED_USER" | grep -qw admin; then

        warn "El usuario ya es administrador"
    ...
    echo -e "${CYAN}Asignando permisos administrador...${RESET}"
    ...
    if [ $? -eq 0 ]; then
        ...
        echo
        echo -e "${GREEN}✔ Usuario ahora es administrador${RESET}"

    else

        echo
        echo -e "${RED}✘ Error asignando permisos admin${RESET}"
...
nc_info_usuario(){
    ...
    echo
    echo -e "${YELLOW}==================================================${RESET}"
    echo -e "${YELLOW}          INFORMACION USUARIO                     ${RESET}"
    echo -e "${YELLOW}==================================================${RESET}"
...
echo
echo -e "${CYAN}====================================================${RESET}"
echo -e "${CYAN}       NEXTCLOUD PRO v7.9.0 PRO VIP GitHub V1.4.2   ${RESET}"
echo -e "${CYAN}====================================================${RESET}"
echo
